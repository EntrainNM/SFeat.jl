...
for i in 1:length(S1) # for each S1 segment
    ...
    start = round(S1[i][1], digits=3) # round to nearest 0.01 (default time step)
    finish = round(S1[i][2], digits=3)
...
try # set S2 a different gender from S1 (anything not female)
    global S2_features = featureFiles[ (!).(contains.(featureFiles, "female")) ][1]
catch # if second gender doesn't exist, use female again
    global S2_features = female_features
end
...
for i in 1:length(S2) # for each S2 segment
    ...
    start = round(S2[i][1], digits=3) # round to nearest 0.01 (default time step)
    finish = round(S2[i][2], digits=3)
...
# savefig(parentFolder*raw"\f0.png")
